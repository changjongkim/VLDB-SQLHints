...
GCP Cloud SQL (MySQL 8.0) 환경에서 **HALO v4**의 성능 개선 효과를 실측한 결과입니다. 특히 클라우드 네이티브 기능인 **Data Cache(로컬 SSD 활용)**와의 상호 작용 및 힌트 최적화의 효과를 심층 분석했습니다.
...
JOB 워크로드(113개 쿼리)를 대상으로 **Baseline(No Cache)**, **Hardware Acceleration(DataCache ON)**, 그리고 **Software Optimization(HALO Hint)** 세 가지 시나리오를 비교 분석했습니다.

#### **12.2.1 전체 처리량 성능 (Overall Throughput)**

| Metric | Baseline (No Cache) | Baseline (With DataCache) | **HALO Hint (No Cache)** |
| :--- | :---: | :---: | :---: |
| **총 실행 시간** | 993.47s | **798.30s** | **808.61s** |
| **성능 향상 (Speedup)** | 1.00x | **1.24x** | **1.23x** |
| **개선율** | - | 19.6% | 18.6% |

> 📌 **핵심 결과**: 별도의 고성능 스토리지 옵션(DataCache) 없이 **HALO 힌트만으로 하드웨어 가속 수준(--0.01x 차이)의 성능을 구현**했습니다.

#### **12.2.2 세부 티어별 개선 실적 (Tier Resolution)**

쿼리 실행 시간에 따른 성능 개선 해상도 분석입니다. HALO는 특히 실행 시간이 긴 Heavy/Medium 쿼리에서 탁월한 병목 해소 능력을 보여줍니다.

| Workload Tier (Baseline) | Count | Baseline Time | HALO Hint Time | **Speedup** |
| :--- | :---: | :---: | :---: | :---: |
...
| Query | Baseline (NoCache) | HALO Hint (NoCache) | **Speedup** | 주요 최적화 내용 |
| :--- | :---: | :---: | :---: | :--- |
...
1.  **소프트웨어 최적화의 한계 극복**: 클라우드 환경에서 I/O 지연은 성능의 가장 큰 적입니다. HALO는 `hint02 (BKA+MRR)` 전략을 적극적으로 추천하여, MySQL 기본 옵티마이저가 사용하는 비효율적인 블록 중첩 루프(BNL) 조인을 배치 키 액세스(BKA)로 전환함으로써 랜덤 I/O 부하를 획기적으로 줄였습니다.
2.  **하드웨어 독립성 증명**: 실험 결과에서 보듯, HALO 힌트는 하드웨어 캐시가 없는 열악한 환경에서도 실행 계획 자체를 최적화함으로써 인프라 업그레이드 비용 없이도 고성능을 유지할 수 있게 합니다. 
3.  **DataCache와의 시너지**: Heavy 쿼리에서의 1.33x 개선은 HALO가 복잡한 조인 순서를 교정하여 스캔 데이터 양을 최소화했기 때문입니다. 이는 추후 DataCache와 결합될 경우 I/O 병목이 아예 사라지는 비약적인 시너지를 낼 것으로 기대됩니다.
