...
| 시나리오 | 저장소 상태 | 총 실행 시간 | **성능 향상 (Speedup)** |
| :--- | :---: | :---: | :---: |
| **Baseline (Native)** | **No Cache** | 10610.56s | 1.00x (Ref) |
| **Baseline (Native)** | **With DataCache** | **5415.99s** | **1.96x** |
| **HALO Hint (Recommended)** | **With DataCache** | **5313.17s** | **2.00x** |

> 📌 **핵심 인사이트**: TPC-H와 같이 I/O 부하가 극심한 환경에서는 **하드웨어 가속(DataCache)이 약 2배의 성능 향상**을 견인했습니다. HALO 힌트는 여기에 추가적인 조인 최적화를 제공하여 **종합 2.00x**의 속도 향상을 달성했습니다.

#### **12.4.2 티어별 상세 분석 (Tier Resolution)**

| Workload Tier (Baseline) | Count | **Baseline (No Cache)** | **HALO Hint (With Cache)** | **Speedup** |
| :--- | :---: | :---: | :---: | :---: |
| **Mega (> 600s)** | 6 | 7306.3s | **3522.1s** | **2.07x** |
| **Heavy (300s - 600s)** | 6 | 2811.9s | **1468.5s** | **1.91x** |
| **Moderate (< 300s)** | 8 | 492.3s | **322.6s** | **1.53x** |


#### **12.4.3 DataCache 상에서의 HALO 추가 개선 (Incremental Gain)**

**중요**: 이 섹션은 **동일하게 하드웨어 가속(DataCache)이 활성화된 환경**에서, MySQL 기본 옵티마이저(Native)와 HALO 힌트 간의 순수 성능 차이를 실측한 결과입니다. 하드웨어가 해결하지 못하는 "라스트 마일" 최적화를 HALO가 담당합니다.

| Query | **Native (With Cache)** | **HALO Hint (With Cache)** | **Incremental Gain (Software)** |
| :--- | :---: | :---: | :---: |
| **q9 (Large Join)** | 1304.0s | **1232.6s** | **+5.8%** |
| **q16 (Aggregation)** | 9.75s | **9.37s** | **+4.1%** |
| **q1 (Scan Heavy)** | 489.8s | **470.8s** | **+4.0%** |
| **q3 (Join Order)** | 563.4s | **550.0s** | **+2.4%** |
