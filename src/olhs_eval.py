# -*- coding: utf-8 -*-
"""
Halo OLHS Scheme Evaluation & Visualization

Simulates the OLHS pipeline:
  ...
  4. Compare against baselines: Native, BAO, Best-Single-Hint, Oracle
...
def olhs_selector(query_speedups_source, query_speedups_target, sigma_threshold=0.05):
    """
    OLHS hint selection:
    ...
    This is a simplified version - full OLHS would use σ to adjust.
    """
    selections = {}
...
def evaluate_strategy(query_speedups, strategy_name, hint_selections=None,
    ...
    Args:
        ...
        strategy_name: label
        hint_selections: dict of qid -> hint_set (for OLHS/BAO)
...
def run_full_evaluation(df_q):
    ...
    for train_env in envs:
        for test_env in envs:
            ...
            # 2. Best-Single-Hint per test env (oracle for single hint)
            best_hs = None
            ...
            bao_sel = olhs_selector(all_speedups[train_env], test_data,
                                    sigma_threshold=0.0)  # No threshold = always try
            results.append(evaluate_strategy(test_data, 'BAO-style', bao_sel))

            # 4. OLHS (trained on train_env, σ-gated)
            olhs_sel = olhs_selector(all_speedups[train_env], test_data,
                                     sigma_threshold=0.05)  # 5% threshold
            results.append(evaluate_strategy(test_data, 'OLHS (Ours)', olhs_sel))

            # 5. Per-query Oracle
            results.append(evaluate_strategy(test_data, 'Oracle'))
...
def visualize_olhs_results(all_results, all_speedups):
    """Generate all OLHS evaluation figures."""
    ...
    for idx, env in enumerate(envs):
        ...
        colors = [COLORS.get('native'), COLORS.get('blue'), COLORS.get('bao'),
                  COLORS.get('olhs'), COLORS.get('oracle')]
    ...
    plt.tight_layout()
    fig.savefig(f'{OUTPUT_DIR}/olhs_home_turf.png')
    plt.close()
    logger.info("Saved: olhs_home_turf.png")
    ...
    for method_idx, method_name in enumerate(['BAO-style', 'OLHS (Ours)', 'Oracle']):
        ax = axes[method_idx]
        matrix = np.zeros((n, n))
    ...
    plt.tight_layout()
    fig.savefig(f'{OUTPUT_DIR}/olhs_cross_hw_transfer.png')
    plt.close()
    logger.info("Saved: olhs_cross_hw_transfer.png")
    ...
    for train_env in envs:
        for test_env in envs:
            ...
            for r in all_results[scenario]:
                ...
                elif r['strategy'] == 'OLHS (Ours)':
                    olhs_regs.append(r['regressions'])
                    olhs_speedups.append(r['overall_speedup'])
    ...
    bars2 = ax.bar(x + width/2, olhs_regs, width, label='OLHS (Ours)',
                   color=COLORS['olhs'], alpha=0.85, edgecolor='white')

    ax.set_xticks(x)
    ax.set_xticklabels(cross_scenarios, fontsize=9, rotation=45, ha='right')
    ax.set_ylabel('Number of Regressions', fontsize=13)
    ax.set_title('Cross-Environment Transfer: Regressions (BAO vs OLHS)',
                 fontsize=16, fontweight='bold')
    ax.legend(fontsize=12, loc='upper left')

    # Add total annotation
    total_bao = sum(bao_regs)
    total_olhs = sum(olhs_regs)
    reduction = (1 - total_olhs / max(total_bao, 1)) * 100
    ax.annotate(f'Total Regressions:\n  BAO: {total_bao}\n  OLHS: {total_olhs}\n  Reduction: {reduction:.0f}%',
               xy=(0.98, 0.98), xycoords='axes fraction',
               ha='right', va='top', fontsize=12,
               bbox=dict(boxstyle='round,pad=0.5', facecolor='lightyellow',
                        edgecolor=COLORS['olhs'], alpha=0.9))

    plt.tight_layout()
    fig.savefig(f'{OUTPUT_DIR}/olhs_regressions.png')
    plt.close()
    logger.info("Saved: olhs_regressions.png")
    ...
    for train_env in envs:
        for test_env in envs:
            if train_env != test_env:
                scenario = f"{train_env} → {test_env}"
                for r in all_results[scenario]:
                    if r['strategy'] == 'OLHS (Ours)' and r['overall_speedup'] > best_olhs_sp:
                        best_olhs_sp = r['overall_speedup']
    ...
    if best_scenario:
        ...
        olhs_r = [r for r in results if r['strategy'] == 'OLHS (Ours)'][0]
        oracle_r = [r for r in results if r['strategy'] == 'Oracle'][0]

        # Left: Per-query speedup comparison
        ax = axes[0]
        olhs_speedups_pq = sorted([pq['speedup'] for pq in olhs_r['per_query']], reverse=True)
        oracle_speedups_pq = sorted([pq['speedup'] for pq in oracle_r['per_query']], reverse=True)

        ax.bar(range(len(olhs_speedups_pq)), olhs_speedups_pq,
               color=COLORS['olhs'], alpha=0.7, label='OLHS')
        ax.plot(range(len(oracle_speedups_pq)), oracle_speedups_pq,
                color=COLORS['oracle'], linewidth=2, alpha=0.8, label='Oracle')
        ax.axhline(y=1.0, color='gray', linewidth=1, linestyle='--', alpha=0.5)
        ax.set_xlabel('Query (sorted by OLHS speedup)', fontsize=12)
        ...
        # Right: Scatter - OLHS vs Oracle speedup per query
        ax = axes[1]
        olhs_pq = {pq['query_id']: pq['speedup'] for pq in olhs_r['per_query']}
        oracle_pq = {pq['query_id']: pq['speedup'] for pq in oracle_r['per_query']}

        common_q = set(olhs_pq.keys()) & set(oracle_pq.keys())
        x_vals = [oracle_pq[q] for q in common_q]
        y_vals = [olhs_pq[q] for q in common_q]

        ax.scatter(x_vals, y_vals, alpha=0.5, s=30, c=COLORS['olhs'], edgecolors='white', linewidth=0.5)
        lim = max(max(x_vals), max(y_vals)) * 1.1
        ax.plot([0, lim], [0, lim], 'k--', linewidth=1, alpha=0.4, label='Perfect = Oracle')
        ax.set_xlabel('Oracle Speedup', fontsize=12)
        ax.set_ylabel('OLHS Speedup', fontsize=12)
        ax.set_title('(b) OLHS vs Oracle (per query)', fontsize=14, fontweight='bold')
        ax.legend(fontsize=11)

        fig.suptitle(f'OLHS Performance: {best_scenario}', fontsize=16, fontweight='bold', y=1.02)
        plt.tight_layout()
        fig.savefig(f'{OUTPUT_DIR}/olhs_per_query.png')
        plt.close()
        logger.info("Saved: olhs_per_query.png")
    ...
    for env in envs:
        ...
        for r in all_results[scenario]:
            if r['strategy'] == 'Native':
                ...
                home_olhs.append(r['overall_speedup'])
            elif r['strategy'] == 'Oracle':
                home_oracle.append(r['overall_speedup'])
    ...
    ax.bar(x, home_olhs, w, label='OLHS', color=COLORS['olhs'], alpha=0.85)
    ax.bar(x + w, home_oracle, w, label='Oracle', color=COLORS['oracle'], alpha=0.8)
    ax.set_xticks(x)
    ...
    methods = ['BAO-style', 'OLHS (Ours)', 'Oracle']
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(['BAO', 'OLHS', 'Oracle'], fontsize=11)
    ax.axhline(y=1.0, color='gray', linewidth=1, linestyle='--', alpha=0.3)
    ...
    total_regs = {m: 0 for m in ['BAO-style', 'OLHS (Ours)']}
    ...
    bars = ax.bar(['BAO-style', 'OLHS (Ours)'],
                  [total_regs['BAO-style'], total_regs['OLHS (Ours)']],
                  color=[COLORS['bao'], COLORS['olhs']], alpha=0.85, edgecolor='white')
    ...
    if total_regs['BAO-style'] > 0:
        reduction = (1 - total_regs['OLHS (Ours)'] / total_regs['BAO-style']) * 100
        ...
        for s, t in storage_scenarios:
            ...
            for r in all_results[scenario]:
                if r['strategy'] == 'BAO-style':
                    ...
                    olhs_sp.append(r['overall_speedup'])

        x = np.arange(len(labels))
        w = 0.3
        ax.bar(x - w/2, bao_sp, w, label='BAO', color=COLORS['bao'], alpha=0.85)
        ax.bar(x + w/2, olhs_sp, w, label='OLHS', color=COLORS['olhs'], alpha=0.85)
        ax.set_xticks(x)
    ...
    avg_home_olhs = np.mean([r['overall_speedup']
                            for s, r_list in all_results.items()
                            for r in r_list
                            if r['strategy'] == 'OLHS (Ours)' and s.split(' → ')[0] == s.split(' → ')[1]])
    avg_cross_olhs = np.mean([r['overall_speedup']
                             for s, r_list in all_results.items()
                             for r in r_list
                             if r['strategy'] == 'OLHS (Ours)' and s.split(' → ')[0] != s.split(' → ')[1]])
    ...
    summary = (
        f"OLHS Key Results\n"
        f"{'━'*28}\n"
        f"Home-turf avg:   {avg_home_olhs:.2f}x\n"
        f"Cross-env avg:   {avg_cross_olhs:.2f}x\n"
        f"BAO cross avg:   {avg_cross_bao:.2f}x\n"
        f"{'━'*28}\n"
        f"OLHS advantage:  {(avg_cross_olhs/avg_cross_bao - 1)*100:+.1f}%\n"
        f"{'━'*28}\n"
        f"BAO regressions: {total_regs['BAO-style']}\n"
        f"OLHS regressions:{total_regs['OLHS (Ours)']}\n"
    ...
    fig.suptitle('OLHS Scheme: Complete Evaluation Summary',
                fontsize=18, fontweight='bold', y=1.01)
    fig.savefig(f'{OUTPUT_DIR}/olhs_grand_summary.png')
    plt.close()
    logger.info("Saved: olhs_grand_summary.png")
...
if __name__ == '__main__':
    ...
    visualize_olhs_results(all_results, all_speedups)
    ...
    with open('/root/halo/results/olhs_evaluation.json', 'w') as f:
        json.dump(summary_data, f, indent=2)

    logger.info(f"\nAll figures saved to: {OUTPUT_DIR}/")
    logger.info("Results JSON: /root/halo/results/olhs_evaluation.json")
